...
+ ヒント
  + 演算子例(演算子とは、足し算とか引き算、掛け算など計算で使われる記号や値の大小を比較する際に使われる記号のことです。)
    + a += b  ⇒　a = a + b
    + a <= b  ⇒　a が b 以下
    + a >= b  ⇒　a が b 以上
...
+ ヒント
  + 書き込み用でファイルオープンするときのモード
    + x: ファイルを新規作成して書き込み(ファイルが存在する場合にはエラー（FileExistsError）となり、ファイルが存在しない場合のみファイルが書き込み用として開かれ保存（新規作成）される。)
    + w: 既存ファイルに書き込み(ファイルが存在しなければ新規作成、存在していれば上書き（既存の内容は削除）で保存される。)
    + a: 既存ファイルに追記(ファイルが存在しなければ新規作成して書き込み、保存される。)
...
```
+ argvでは0番目の要素に.pyファイル名が入り、1番目以降の要素にコマンドライン引数が入ります。
+ 今回のまとめコードでは、1番目の要素、すなわちコマンドライン引数として1番目に指定したファイルを開く処理を行っています。
```python
    f = open(sys.argv[1], "r",encoding="utf-8")
```
---
...
+ 今回のコードでは、この要素数が2以外の場合は標準エラー出力し、プログラムを終了するようにしています。
