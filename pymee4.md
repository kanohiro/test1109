...
if len(sys.argv) != 2:
    ...
    sys.exit(1)
```
上記では、len(sys.argv) で引数の要素数をチェックしています。
...
print(value)
```
+ 出力結果（引数にtest.txtを指定して実行）
```
['argv.py', 'test.txt']

```

argv では0番目の要素に.pyファイル名が入り、1番目以降の要素に引数が入るので、<br>
.pyファイルを実行するときの引数を1つにする場合、要素数は"2"になります。<br>
...
# with構文
with構文は主に外部のファイルを扱う場面で活躍する構文になります。<br>
