...
```python
    ...
    line = f.readline()
```
+ 引数で指定したファイルを読み取り専用で開いて変数fとして管理し、その内容をreadline関数で1行ずつ読み込んでいます。
...
+ sysモジュールのargvでコマンドライン引数（.pyファイルを実行するときの引数）の値を受け取るプログラムを作ってみます。
...
+ argvでは0番目の要素に.pyファイル名が入り、1番目以降の要素に引数が入るので、.pyファイルを実行するときの引数（コマンドライン引数）を1つにする場合、要素数は"2"になります。
